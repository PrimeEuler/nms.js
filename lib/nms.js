...
function nms(STDIN,STDOUT){
            ...
            radius : {
                "Access-Request":function(username,password){
                    ...
                    radiusClient["Access-Request"](NMS.cli.access, function(resp){
                        NMS.cli.access.user = resp.user
                        NMS.cli.access.uuid = resp.uuid
                        NMS.cli.access.code = resp.code
                        if(resp.code === 'Access-Reject'){
                            NMS.cli.access.rejects++;
                        }
                        if(resp.code === 'Access-Accept'){
                            NMS.cli.access.rejects = 0;
                        }
                        if(NMS.cli.access.rejects >= 3){
                           NMS.context.exit();
                           return;
                        }
        ...
        NMS.access = NMS.context.nms.radius["Access-Request"];
